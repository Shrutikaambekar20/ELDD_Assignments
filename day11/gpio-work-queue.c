...
static __init int GPIO_init(void)
{
	...
	ret = request_irq(irq, switch_isr, IRQF_TRIGGER_RISING, "SWITCH", NULL);
	if (ret != 0)
	{
...
static ssize_t GPIO_write(struct file *pfile, const char __user *ubuf, size_t size, loff_t *poffset)
{
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Shrutika Ambekar <shrutikaambekar26@gmail.com>");
